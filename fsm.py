...
class TocMachine(GraphMachine):
    ...
    def is_going_to_state1(self, event):
        if event.get("message"):
            ...
            if(int(text) < 0 or int(text) > 12):
                ...
                send_text_message(sender_id, "您輸入的月份格式不符合")
                return False
            else:
                self.month = int(text)
                return True
    ...
    def on_enter_state3(self, event):
        ...
        #send_text_message(sender_id, responese_str)
    ...
    def on_enter_state4(self, event):
        ...
        for i in range(0, self.num_games):
            responese_str = responese_str + str(i + 1) + "\n"
            responese_str = responese_str + self.today_scores[i * 2]['TAME_ABBREVIATION'] + " " + str(self.today_scores[i]['PTS']) + "\n"
            responese_str = responese_str + self.today_scores[i * 2 + 1]['TAME_ABBREVIATION'] + " " + str(self.today_scores[i * 2 + 1]['PTS']) + "\n"
    ...
    def on_enter_state5(self, event):
        ...
        responese_str = "TEAM   1   2   3   4   ALL\n"
        responese_str = responese_str + team_A['TAME_ABBREVIATION'].ljust(5) + " " + str(team_A['PTS_QTR1']).ljust(2) + " " + str(team_A['PTS_QTR2']).ljust(2) + " " + str(team_A['PTS_QTR3']).ljust(2) + " " + str(team_A['PTS_QTR4']).ljust(2) + " " + str(team_A['PTS']) + "\n"
        responese_str = responese_str + team_B['TAME_ABBREVIATION'].ljust(5) + " " + str(team_B['PTS_QTR1']).ljust(2) + " " + str(team_B['PTS_QTR2']).ljust(2) + " " + str(team_B['PTS_QTR3']).ljust(2) + " " + str(team_B['PTS_QTR4']).ljust(2) + " " + str(team_B['PTS'])

        sender_id = event['sender']['id']
        send_text_message(sender_id, responese_str)
        responese = send_text_message(sender_id, "請輸入\'scores\' 或 \'today scores\' 或 \'team\'")
        self.go_back()
